...
from gi.repository import GLib, Gst, GstRtp, GstRtsp  # noqa: E402
...
class CombinedRTSPClient:
    ...
    def __init__(
        ...
        *,
        ...
        stream_session_metadata_callback: Optional[
        ...
        shared_memory: mp.shared_memory.SharedMemory | None = None,
    ) -> None:
    ...
    def _create_rtspsrc(self) -> None:
        ...
        src.props.protocols = GstRtsp.RTSPLowerTrans.TCP | GstRtsp.RTSPLowerTrans.UDP

        # Be stricter about timeout handling
        src.props.tcp_timeout = 100_000_000  # µs until we declare the server dead

        # 'Axis Scene Description' (the application data) streams sometimes
        # arrive late; delay EOS
    ...
    def _on_new_video_sample(self, sink: Gst.Element) -> Gst.FlowReturn:
        ...
        if self.shared_mem is not None:
            ...
            nbytes = frame_c.nbytes
            if nbytes > self.shared_mem.size:
                self._report_error(
    ...
    def _on_new_application_data_sample(self, sink: Gst.Element) -> Gst.FlowReturn:
        ...
        if not ok:
            self._report_error(
                "Application Data Buffer", "Failed to map application data buffer"
            )
            return Gst.FlowReturn.ERROR
        ...
        if len(raw) < hdr_len:
            self._report_error(
                "RTP Header",
                f"Incomplete RTP header: expected {hdr_len} bytes, got {len(raw)}",
            )
            return Gst.FlowReturn.ERROR
    ...
    def _report_error(
        self, error_type: str, message: str, exception: Optional[Exception] = None
    ) -> None:
        ...
        while True:
            try:
                ...
                if item["kind"] == "video":
                    ...
                    print(
                        f"ERROR: {item.get('error_type', 'Unknown')}: "
