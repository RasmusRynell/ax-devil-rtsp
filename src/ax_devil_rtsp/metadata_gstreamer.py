...
logger = logging.getLogger("AxisMetadataClient")
logging.basicConfig(
    level=logging.INFO,
...
class AxisMetadataClient:
    ...
    def _on_pad_added(self, src, pad):
        ...
        if media_type != "application":
            logger.info("Ignoring pad '%s' with media type '%s'", pad.get_name(), media_type)
            return
        ...
        if sink_pad and not sink_pad.is_linked():
            ...
            logger.info("Jitterbuffer sink pad is already linked or unavailable.")
            ...
            if start_index == -1:
                ...
                try:
                    ...
                    if not xml_text.lstrip().startswith("<"):
                        logger.error("Fallback decoded XML does not appear valid. Discarding payload.")
                        self.xml_buffer = b""
                        buffer.unmap(map_info)
                ...
                if not xml_text.lstrip().startswith("<"):
                    ...
                    logger.info("Complete XML metadata received (length: %d).", len(xml_text))
